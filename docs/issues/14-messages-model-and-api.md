...
1. Migration per DESIGN §7.2: `messages` with index (roomId, id DESC),
   plus `dedupeId text?` and unique index `(senderId, dedupeId)` (nullable
   dedupe ignored). Body ≤ 4000 enforced in zod + DB check constraint.
2. Send pipeline (`sendText({ spaceId, roomId, body, dedupeId? })`,
   `sendImage({ spaceId, roomId, attachmentId, caption?, dedupeId? })`):
   order per DESIGN §8.4 — auth → `canAccessRoom` **as member** (observers
   cannot send; guardians must join to talk — explicit rule + test) →
   archived check → quiet-hours hook (`assertNotQuiet(ctx)` no-op until 29)
   → zod → moderation hook (`moderateContent()` returns `clean` until 27) →
   insert → post-commit hooks `onMessageCreated(message)` (WS in 15, notify
   in 37 — register-pattern like `onSpaceCreated`).
...
3. Dedupe: same `(senderId, dedupeId)` within retention returns the
   original message (idempotent success, no duplicate row) — implement via
   unique-violation catch → fetch existing.
4. `list({ spaceId, roomId, before?, limit≤50 })` — `canAccessRoom`
   (member or observer); descending by id; returns `MessageDTO[]` +
   `nextCursor`. `around({ messageId })` — 25 before/after for deep links.
...
7. System messages: internal `postSystemMessage(roomId, key, params)`
   service (used by 13's membership maintenance for join/leave notices —
   wire those two call sites now); body stores `{ key, params }` JSON;
   clients render localized.
...
- [ ] Lifecycle matches DESIGN §10.2 for clean/deleted paths (flag paths in
      27); hooks in the exact §8.4 order (asserted by an order-recording
      test double).
