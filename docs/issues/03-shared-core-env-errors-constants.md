...
2. `src/env.ts`: zod schemas + loader per process, exactly the DESIGN §3.4
   matrix: `apiEnvSchema`, `workerEnvSchema` (both server-side, with
   required/optional/defaults as specified) and `loadEnv(schema)` that
   parses `process.env`, and on failure prints every missing/invalid key
   then exits(1). No secret defaults. `NODE_ENV` enum
...
6. `src/audit.ts`: `AUDIT_ACTIONS` string-literal union exactly per DESIGN
   §13.8, plus `GUARDIAN_VISIBLE_ACTIONS` subset list.
7. `src/ids.ts`: `newId()` returning a ULID (dependency `ulid`), and
   `isId(s)` validator (26-char Crockford base32 regex).
8. Unit tests: env loader failure lists all bad keys; error codes unique;
   limits table matches DESIGN §19.5 values (hardcoded expectations);
   `newId()` ordering is lexicographically increasing across 1000 sequential
   calls.
...
```bash
pnpm --filter @famchat/shared typecheck && pnpm --filter @famchat/shared test
node -e "const{newId}=require('./packages/shared/dist/ids.js');console.log(newId())"
```
